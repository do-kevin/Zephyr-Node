...
class PlayCards extends React.Component {
	...
	getDeckAppts = () => {
		axios.get(`/appointments/decks/${this.state.deckId}/${this.state.userId}`).then((data) => {
			console.log(data.data);
			this.setState({
				appts: [ ...data.data ]
			});
	...
	deleteFlashcard = (id) => {
		axios.delete('/flashcards/' + id).then((data) => {
			console.log(data);
			this.getFlashcards();
			axios.delete('/appointments/decks/' + id).then((response) => {
	...
	saveFlashcardChanges = (id, index) => {
		...
		axios.put('/flashcard/' + id, obj).then((data) => {
			console.log(data);
			this.getFlashcards();
			axios.put('/appointments/flashcard/' + id, frontObj).then((response) => {
				console.log(response);
			});
			axios.put('/appointments/flashcard/' + id, backObj).then((response) => {
				console.log(response);
				this.setState({
					editSaved: true,
					editId: index
	...
	saveSettings = () => {
		...
		axios.put('/decks/' + this.state.deckId, obj).then((data) => {
			...
			if (obj.dailyQuiz === true && this.state.deck.dailyQuiz === false) {
				...
				axios.delete(`/appointments/decks/${this.state.deckId}/${this.state.userId}`).then(() => {
					this.getDeckAppts();
					if (obj.dailyQuiz) {
						this.createAppointments();
					}
	...
	render() {
		...
		if (openSettings === true) {
			modal = (
				<div>
					<Modal isOpen={this.state.openSettings} toggle={this.openSettings} className={this.props.className}>
