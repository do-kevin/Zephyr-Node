  ...
  render() {
        ...
        <div className="scrolldown-cards">
          <ListGroup className="todo-listgroup">
            {this.state.reminders.map((reminder) => {
              ...
              var time = moment(
                reminder.date.replace(":00.000Z", "").replace("T", " ")
              ).format("hh:mma");
              return (
                <ListGroupItem
                  className="profile-reminders animated flipInX"
                  key={reminder.id}
                >
    ...
    if (this.state.decks.length > 1) {
      renderDecks = (
        <div style={{ display: "flex", flexWrap: "wrap" }}>
          {this.state.decks
            .slice(Math.max(this.state.decks.length - 4, 1))
            .map((deck) => {
              return (
                <Col key={deck.id}>
                  <div className="decks decks-primary animated bounceIn">
                    <Link
                      to="/deck"
                      onClick={() => this.deckIdSessionStorage(deck.id)}
                    >
                      <div className="container">
                        <h1 className="deck-title text-center">
                          {deck.subject}
                        </h1>
                      </div>
                    </Link>
      ...
      renderDecks = (
        <Col>
          <ListGroup>
            <ListGroupItem className="noDisplay">
    ...
    if (this.state.notes.length > 1) {
      renderNotes = (
        <div>
          {this.state.notes
            .slice(Math.max(this.state.decks.length - 2, 1))
            .map((note) => {
              return (
                <section
                  key={note.id}
          ...
          <Main>
            ...
            <div id="reminders-todos" className="page-block">
              <section className="page-block__reminders page-block--clear">
                ...
                {renderReminders}
                <Link to="/reminder">
                  <button className="profile-btns">View reminders</button>
                </Link>
              </section>
              <section className="page-block__todos page-block--clear">
                ...
                {renderToDos}
                <Link to="/todo">
                  <button className="profile-btns">View tasks</button>
                </Link>
              </section>
            </div>
            <h2>Recent Decks</h2>
